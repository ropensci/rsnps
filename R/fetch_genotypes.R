...
#'
#' @export
#' @param url (character) URL for the download. See example below of function use.
#' @param rows (integer) Number of rows to read in. Useful for getting a glimpse of 
#' the data. Negative and other invalid values are ignored, giving back all data. 
#' Default: 100
#' @param filepath (character) If none is given the file is saved to a temporary file, 
#' which will be lost after your session is closed. Save to a file if you want to 
#' access it later.
#' @param quiet (logical) Should download progress be suppressed. Default: \code{TRUE}
...
fetch_genotypes <- function(url, rows = 100, filepath = NULL, quiet = TRUE, ...) {
  ...
  download.file(url, destfile = filepath, quiet = quiet, ...)
  df <- read.table(filepath, sep = "\t", nrows = rows, header = FALSE, 
                   comment.char = "#", stringsAsFactors = FALSE)
  setNames(df, c("rsid", "chromsome", "position", "genotype"))
}
