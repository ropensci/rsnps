...
### Query with conversion of addresses from dbSNP GRCh38 (standard) to hg19 via liftOver

This uses the '2' form of the query, which includes build metadata.
```{r eval=TRUE}
library(rsnps)
data(Hg38toHg19.chain) 
S4Vectors::metadata(Hg38toHg19.chain)
snps <- c("rs6060535", "rs7001")
ncbi_snp_query2( snps )
# now query with conversion
ncbi_snp_query2( snps, chain=Hg38toHg19.chain)
```
